...
def debug_setup(scenario: str, game: GameState = Depends(get_current_game)):
    ...
    # --- 新規追加した隠し実績のテストケース ---
        ...
        # CPUの手牌 (不自然にならない程度にバラけさせつつ、各牌4枚の制限を厳守)
        ...
        # プレイヤーが奇数牌をツモりやすいように山札に仕込む（右端からツモる）
        # Player: 4p(ハズレ) -> 1s(アタリ) -> 8p(ハズレ) -> 3s(アタリ) -> 5s(アタリ) -> 9p(アタリ) -> 1p(和了)
        game.wall = [
            "2s", "4s", "6s", "8s", "1s", "5s", "9s", # 予備の山札
            "1p", # Pツモ7 (和了)
            "9m", "9m", "9m", # C3, C2, C1ツモ
            "9p", # Pツモ6
            "9m", "1m", "1m", # C3, C2, C1ツモ
        ...
        # プレイヤーが数ターンの間に鳴ける牌をCPUに引かせる積み込み
        game.wall = [
            "南", "西", "北", "中", "1s", "1p", "3s", "東", "1s", "9m", # 予備
            "發", # C3ツモ -> P0ロン(春)
            "2s", # C2ツモ
            "白", # C1ツモ -> P0ポン
            "春", # P0ツモ2 (ハズレ)
